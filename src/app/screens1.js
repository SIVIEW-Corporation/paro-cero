...
export function Dashboard({ wo }) {
  const open = wo.filter(
    (w) => !['completado', 'cancelado'].includes(w.status),
  ).length;
  ...
  const upcoming = wo.filter((w) =>
    ['pendiente', 'asignado'].includes(w.status),
  );
