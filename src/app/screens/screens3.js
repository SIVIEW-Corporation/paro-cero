        ...
        >
          {filteredChecklists.map((c) => (
            <tr key={c.id}>
              ...
              <Td>
                <Badge
                  label={CHECKLIST_ESTADOS[c.estado]}
                  color={CHECKLIST_ESTADO_COLORES[c.estado]}
                />
              </Td>
              <Td>
                {c.estado === 'pendiente' && (
...
function ExecuteChecklistScreen({
  ...
  const handleValorChange = (itemId, valor) => {
    setItems((prev) =>
      prev.map((i) =>
        i.itemId === itemId
  ...
  const handleNotaChange = (itemId, nota) => {
    setItems((prev) =>
...
function CreateChecklistForm({ plantillas, onClose, onSave }) {
...
function CreatePlantillaForm({ onClose, onSave }) {
  const [nombre, setNombre] = useState('');
  const [activoId, setActivoId] = useState('');
  const [items, setItems] = useState(
    CHECKLIST_ITEMS_DEFAULT.map((i) => ({ ...i })),
  );
...
function CreateHallazgoForm({ onClose, onSave }) {
  const [activoId, setActivoId] = useState('');
